...
class TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(new Offset(0, mainLineY), new Offset(size.width, mainLineY),
        mainLinePaint);

    double currTimeX = size.width * animationValue;
    canvas.drawLine(
        new Offset(currTimeX, labelHeightDoubled),
        new Offset(currTimeX, size.height - labelHeightDoubled),
        milestoneLinePaint);

    {
      for (int week = 0; week < numWeeks; week++) {
        ...
        if (lineHeight > 0) {
          ...
          canvas.drawLine(new Offset(currX, margin),
              new Offset(currX, size.height - margin), mainLinePaint);
        }

        if (isYear) {
          var yearLabel = "${yearNumber}";
          labelPainters[yearLabel]
              .paint(canvas, new Offset(currX, size.height - labelHeight));
          yearNumber++;
        }
      }
    }

    {
      for (int i = 0; i < weekLabels.length; i++) {
        ...
        double margin = (size.height - lineHeight) / 2;
        canvas.drawLine(new Offset(currX, margin),
            new Offset(currX, size.height - margin), milestoneLinePaint);

        textPainter.paint(
            canvas, new Offset(currX, size.height - labelHeightDoubled));
      }
    }
  }
